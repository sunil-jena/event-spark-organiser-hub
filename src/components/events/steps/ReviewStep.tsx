...
export const ReviewStep: React.FC<ReviewStepProps> = ({ eventData, onSubmit, onBack }) => {
  ...
  const bannerImage =
    eventData.media.eventBannerImage && eventData.media.eventBannerImage.length > 0
  ...
  const renderDatesAndTimeSlots = () => {
    ...
    return eventData.dates.map(date => {
      ...
      const end = date.endDate ? format(convertNumberToDate(date.endDate), 'PPP') : null;
      const slots = eventData.timeSlots.filter(slot => slot.dateId === date.id);
      return (
        ...
        <tbody>
          {eventData.tickets.map(ticket => (
            <tr key={ticket.id} className="border-b">
              ...
              <td className="p-2">
                {ticket.ticketType === 'free' ? 'Free' : formatIndianPrice(ticket.price)}
              </td>
      ...
      <CardContent className="p-0">
        ...
        <div className="relative">
          ...
          <div className="absolute bottom-4 left-4 flex gap-2">
            <Badge className="bg-primary/80 text-white">{basicDetails.category || 'Category'}</Badge>
            {basicDetails.isOnline && (
              <Badge variant="outline" className="bg-blue-500/80 text-white">Online Event</Badge>
            )}
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            <h2 className="text-2xl font-bold">{basicDetails.title || 'Event Title'}</h2>
            ...
            {basicDetails.eventHighlights && basicDetails.eventHighlights.length > 0 && (
              <div className="mt-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {media.eventVerticalCardImage && (
                <div>
                  <h4 className="text-sm font-medium text-gray-500 mb-2">Vertical Card Image</h4>
                  <img
            ...
            {media.youtubeLink && (
              <div className="mt-4">
                ...
                <div className="flex items-center border p-2 rounded-md justify-between">
                  ...
                  <Button type="button" size="sm" variant="ghost" onClick={() => window.open(media.youtubeLink, '_blank')}>
            ...
            {additionalInfo ? (
              <div className="space-y-4">
                {additionalInfo.termsAndConditions && (
                  <div>
                    <h4 className="text-lg font-medium text-gray-500 mb-1">Terms & Conditions</h4>
                    <p className="text-sm whitespace-pre-wrap">{additionalInfo.termsAndConditions}</p>
                  </div>
                )}
                {additionalInfo.faqItems && additionalInfo.faqItems.length > 0 && (
                  <div>
                    <h4 className="text-lg font-medium text-gray-500 mb-1">FAQ</h4>
                ...
                {additionalInfo.prohibitedItems && additionalInfo.prohibitedItems.length > 0 && (
                  <div>
                    <h4 className="text-lg font-medium text-gray-500 mb-1">Prohibited Items</h4>
                    <div className="flex flex-wrap gap-2">
