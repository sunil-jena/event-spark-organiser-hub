...
export const AdditionalInfoStep: React.FC<AdditionalInfoStepProps> = ({
  ...
  const addFaqItem = () => {
    const faqItems = [...formik.values.faqItems || [], { question: "", answer: "" }];
    ...
    <Card className="shadow-lg border-[#24005b]/10">
      <CardContent className="pt-6">
        <form onSubmit={formik.handleSubmit} className="space-y-8">
          <h2 className="text-2xl font-semibold mb-6 text-[#24005b]">Additional Information</h2>

          <div className="space-y-8">
            {/* Terms and Conditions with Rich Text Editor */}
            <div className="space-y-3">
              <Label htmlFor="termsAndConditions" className="text-lg font-medium">
                Terms & Conditions
              </Label>
              <p className="text-sm text-gray-500">
                Add all terms, FAQs, event rules, and refund policies in one place.
              </p>
              <RichTextEditor
                value={formik.values.termsAndConditions || ''}
                onChange={(content) => formik.setFieldValue('termsAndConditions', content)}
            ...
            <Collapsible open={isFaqOpen} onOpenChange={setIsFaqOpen} className="border rounded-md p-4">
              ...
              <CollapsibleContent className="pt-4 space-y-4">
                <p className="text-sm text-gray-500">
                  Add frequently asked questions that will appear in a dedicated section.
                </p>
                {formik.values.faqItems && formik.values.faqItems.map((faq, index) => (
                ...
                {/* FAQ Preview */}
                {formik.values.faqItems.filter(item => item.question && item.answer).length > 0 && (
            ...
            <ProhibitedItemsSection
              ...
              onChange={(items) => formik.setFieldValue('prohibitedItems', items)}
            ...
            <SponsorSection
              ...
              onChange={(sponsors) => formik.setFieldValue('sponsors', sponsors)}
            />
          </div>
